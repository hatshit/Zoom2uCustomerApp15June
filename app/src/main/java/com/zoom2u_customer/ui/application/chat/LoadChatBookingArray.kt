...
class LoadChatBookingArray(arrayOfChat :MutableList<ChatList>) {
    ...
    init {
        ...
        if (arrayOfChatDelivery.size > 0) {
            for (model_deliveriesToChat in arrayOfChatDelivery) {
                FirebaseDatabase.getInstance().reference.child("customer-courier-booking-chat/" + model_deliveriesToChat.bookingId.toString() + "_" + model_deliveriesToChat.courierId.toString() + "/status/courier/unread")
                    .removeEventListener(model_deliveriesToChat.valueEventListnerOfCourierChat!!)
                FirebaseDatabase.getInstance().reference.child("customer-courier-booking-chat/" + model_deliveriesToChat.bookingId.toString() + "_" + model_deliveriesToChat.courierId.toString() + "/status/customer/unread")
    ...
    companion object {
        var arrayOfChatDelivery: MutableList<ChatList> = ArrayList()
        var customerID = ""
